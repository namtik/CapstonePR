...
0. **카드 사용 중앙 연출 + 효과 지연** — 카드 사용 시 사용한 카드가 화면 중앙에 잠깐 떠올랐다(등장→유지→) 사라짐. 두 가지 모드:
   - **일반 사용(차단/효과 지연)**: 카드가 **사라지는 순간 실제 효과가 실행**됨. `OnUseCardRequested`(일반 경로)를 **둘로 분리** — 즉시 `PullFromHand`+연출 시작 → 연출 종료 콜백에서 `ResolveCardUse(card)`가 효과/게이지/각성카운트 등 실행. `_cardPresenting` 플래그로 연출 중 다른 카드 사용 차단(효과 직렬화). ⚠️ 향후 카드 사용 로직 수정 시 **효과는 연출 종료 시점에 실행됨**에 유의
   - **각성 중(비차단/교체)**: 콤보 연타를 막지 않도록 `PlayCardUsePresentation(card, null, interruptable:true)` — 효과(드로우+콤보입력)는 즉시 실행되고 연출은 순수 시각용. 빠르게 연속 입력하면 직전 연출을 교체해 항상 최신 카드만 중앙 표시(`_interruptablePresentCo/View`로 추적).
   - 연출 구현: `CardHandHUD.PlayCardUsePresentation()`(임시 카드 뷰를 중앙에 생성→페이드/스케일→Destroy). 튜닝: `CardHandHUD`의 `cardUsePresent*`(위치/배율/등장·유지·퇴장 시간/ON-OFF)
