...
public class RelicStageController : MonoBehaviour
{
    ...
    System.Collections.IEnumerator PlayChestOpenSequence()
    {
        ...
        if (!rewardGranted)
        {
            rewardGranted = TryGrantRewardAndPickSprite(out grantedRewardSprite);
    ...
    bool TryGrantRewardAndPickSprite(out Sprite rewardSprite)
    {
        rewardSprite = null;

        if (RelicManager.Instance == null)
        {
            Debug.LogWarning("[RelicStage] RelicManager.Instance가 없어 유물 지급을 건너뜁니다.");
            return false;
        }
        ...
        if (picked.isSpriteOnly)
        {
            ...
            {
                rewardSprite = picked.sprite;
                return false;
            }

            rewardSprite = granted != null && granted.icon != null ? granted.icon : picked.sprite;
