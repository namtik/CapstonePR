...
{
    /// <summary>
    /// ComboSkill_DB.xlsx → Assets/Resources/ComboDB/*.json 변환기 (Unity Editor 메뉴).
    /// CardDBImporter와 동일한 워크플로우/파서. 외부 라이브러리 없이 .NET 표준만 사용.
    /// 시트: ComboSkills, ComboEffects (키값_한글정리는 무시).
    /// </summary>
    public static class ComboSkillDBImporter
    {
        const string PREF_LAST_PATH = "Battle.ComboDB.LastSourcePath";
        const string OUT_DIR = "Assets/Resources/ComboDB";

        [MenuItem("Tools/Combo DB/Excel → JSON 변환...", priority = 10)]
        public static void ConvertFromExcelMenu()
        {
            string lastPath = EditorPrefs.GetString(PREF_LAST_PATH, "");
            string startDir = (!string.IsNullOrEmpty(lastPath) && File.Exists(lastPath))
                ? Path.GetDirectoryName(lastPath)
                : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads");

            string path = EditorUtility.OpenFilePanel("ComboSkill_DB.xlsx 선택", startDir, "xlsx");
        ...
        [MenuItem("Tools/Combo DB/마지막 파일 다시 변환", priority = 11)]
        public static void ConvertFromLastPath()
        {
            string lastPath = EditorPrefs.GetString(PREF_LAST_PATH, "");
            if (string.IsNullOrEmpty(lastPath) || !File.Exists(lastPath))
            {
                EditorUtility.DisplayDialog("Combo DB",
                    "최근에 변환한 파일이 없거나 존재하지 않습니다.\n'Excel → JSON 변환...' 메뉴로 먼저 선택하세요.", "확인");
                return;
            }
            ConvertFromExcel(lastPath);
        }

        [MenuItem("Tools/Combo DB/마지막 파일 다시 변환", validate = true)]
        ...
        [MenuItem("Tools/Combo DB/출력 폴더 열기", priority = 50)]
        ...
        {
            int skillCount = 0, effectCount = 0;
            try
            {
                EditorUtility.DisplayProgressBar("Combo DB", $"파싱: {Path.GetFileName(xlsxPath)}", 0.2f);
                var workbook = ExcelReader.Read(xlsxPath);

                if (!workbook.Sheets.TryGetValue("ComboSkills", out var skillsSheet))
                    throw new Exception("Excel에 'ComboSkills' 시트가 없습니다.");
                if (!workbook.Sheets.TryGetValue("ComboEffects", out var effectsSheet))
                    throw new Exception("Excel에 'ComboEffects' 시트가 없습니다.");

                EditorUtility.DisplayProgressBar("Combo DB", "JSON 생성 중...", 0.5f);
                string skillsJson = BuildSkillsJson(skillsSheet, out skillCount);
                string effectsJson = BuildEffectsJson(effectsSheet, out effectCount);

                Directory.CreateDirectory(OUT_DIR);
                var enc = new UTF8Encoding(false);
                File.WriteAllText(Path.Combine(OUT_DIR, "ComboSkills.json"), skillsJson, enc);
                File.WriteAllText(Path.Combine(OUT_DIR, "ComboEffects.json"), effectsJson, enc);

                EditorUtility.DisplayProgressBar("Combo DB", "Asset 새로고침...", 0.9f);
                AssetDatabase.Refresh();
                EditorUtility.ClearProgressBar();

                string msg = $"변환 완료\n• 콤보 {skillCount}개\n• 효과 {effectCount}건\n" +
                             $"출력: {OUT_DIR}/ComboSkills.json + ComboEffects.json";
                Debug.Log($"[ComboDB] {msg.Replace("\n", " ")}");
                EditorUtility.DisplayDialog("Combo DB", msg, "확인");
            }
            catch (Exception e)
            {
                EditorUtility.ClearProgressBar();
                Debug.LogError($"[ComboDB] 변환 실패: {e}");
                EditorUtility.DisplayDialog("Combo DB 변환 실패", e.Message, "확인");
            }
        }

        // ─── ComboSkills 시트 → JSON ──────────────────────────────

        static string BuildSkillsJson(SheetData sheet, out int count)
        {
            count = 0;
            var hdr = sheet.HeaderIndex;
            int idCol      = RequireCol(hdr, "ComboID");
            int slot1Col   = RequireCol(hdr, "Slot1");
            int slot2Col   = RequireCol(hdr, "Slot2");
            ...
            foreach (var row in sheet.DataRows)
            {
                ...
                sb.AppendFormat("      \"slot1\": {0},\n", JsonStr(GetCell(row, slot1Col).Trim().ToUpperInvariant()));
                sb.AppendFormat("      \"slot2\": {0},\n", JsonStr(GetCell(row, slot2Col).Trim().ToUpperInvariant()));
        ...
        // ─── ComboEffects 시트 → JSON ─────────────────────────────

        struct EffectField
        {
            public string Header, JsonKey; public bool IsInt;
            public EffectField(string h, string j, bool i) { Header = h; JsonKey = j; IsInt = i; }
        }

        static readonly EffectField[] EFFECT_FIELDS =
        {
        ...
        static string BuildEffectsJson(SheetData sheet, out int count)
        {
            ...
            foreach (var row in sheet.DataRows)
            {
                if (!TryGetInt(row, refCol, out int refId)) continue;
                if (!first) sb.Append(",");
                first = false;
                sb.Append("\n    {\n");
                sb.AppendFormat("      \"refComboId\": {0}", refId);
        ...
        {
            if (!hdr.TryGetValue(key, out int idx)) throw new Exception($"필수 컬럼 '{key}' 없음");
            return idx;
        }
        static int TryCol(Dictionary<string, int> hdr, string key) => hdr.TryGetValue(key, out int idx) ? idx : -1;
        static string GetCell(List<string> row, int col) => (col < 0 || col >= row.Count) ? "" : (row[col] ?? "");
        ...
        // ─────────────────────────────────────────────────────────────
        // 최소 xlsx 파서 (zip + XML, 외부 의존성 없음) — CardDBImporter와 동일
        // ─────────────────────────────────────────────────────────────

        class SheetData
        {
            public List<List<string>> Rows = new List<List<string>>();
            public Dictionary<string, int> HeaderIndex = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        ...
        class WorkbookData
        {
            public Dictionary<string, SheetData> Sheets = new Dictionary<string, SheetData>(StringComparer.Ordinal);
        }
        ...
        static class ExcelReader
        {
            ...
            public static WorkbookData Read(string path)
            {
                ...
                using (var zip = new ZipArchive(fs, ZipArchiveMode.Read))
                {
                    ...
                    foreach (var r in relsXml.Root.Elements(NS_PKG + "Relationship"))
                    {
                        ...
                        if (idAttr != null && tgtAttr != null) relsMap[idAttr.Value] = tgtAttr.Value;
                    }
                    ...
                    foreach (var sheetEl in sheetsRoot.Elements(NS_MAIN + "sheet"))
                    {
                        ...
                        if (string.IsNullOrEmpty(name) || !relsMap.TryGetValue(rId, out string target)) continue;
            ...
            static List<string> ReadSharedStrings(ZipArchive zip)
            {
                ...
                {
                    var sb = new StringBuilder();
                    foreach (var t in si.Descendants(NS_MAIN + "t")) sb.Append(t.Value);
                ...
                foreach (var rowEl in sheetData.Elements(NS_MAIN + "row"))
                {
                    ...
                    foreach (var cellEl in rowEl.Elements(NS_MAIN + "c"))
                    {
                        ...
                        if (type == "s")
                        {
                            ...
                            if (v != null && int.TryParse(v.Value, out int idx) && idx >= 0 && idx < sharedStrings.Count)
                        ...
                        else if (type == "inlineStr")
                        {
                            ...
                            {
                                var sb = new StringBuilder();
                                foreach (var t in inl.Descendants(NS_MAIN + "t")) sb.Append(t.Value);
                                value = sb.ToString();
                            }
                        }
                        else if (type == "b")
